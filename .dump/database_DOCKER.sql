...
-- File: database_DOCKER.sql
-- This script is designed to be run by Docker's entrypoint (e.g., in /docker-entrypoint-initdb.d/)
-- It assumes the 'dukcapil_ktp' database is already created by the Docker entrypoint.

-- IMPORTANT: The DROP DATABASE, CREATE DATABASE, and \connect commands have been removed.
...
-- 1. Drop problematic view first (if it exists from previous attempts)
DROP VIEW IF EXISTS ktp_summary CASCADE;

-- 2. Drop existing table to recreate fresh (important for clean re-initialization)
-- This ensures a clean slate for the table structure on each initialization.
